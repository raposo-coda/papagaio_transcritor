(() => {
  ...
  async function loadConfig() {
    ...
    state.summaryEnabled = cfg.summary_enabled !== false;
  ...
  function renderSummaryState() {
    el("summary-toggle").setAttribute("aria-checked", String(state.summaryEnabled));
    const aviso = el("summary-aviso");
    if (state.summaryEnabled) {
      aviso.classList.add("hidden");
      return;
    }
    // So aparece quando esta desligado: e o estado que muda o que sai no fim.
    aviso.textContent =
      state.mode === "cloud"
        ? "So as transcricoes. O consolidado sai como indice, e o Gemini nao e chamado para resumir."
        : "So as transcricoes. O consolidado sai como indice, sem o panorama do conteudo.";
    aviso.classList.remove("hidden");
  }

  async function onSummaryChange() {
    const ligado = !state.summaryEnabled;
    state.summaryEnabled = ligado;
    if (await postConfig({ summary_enabled: ligado })) renderSummaryState();
  }
  ...
  async function startJob() {
    ...
    formData.append("summary_enabled", state.summaryEnabled);
  ...
  el("summary-toggle").addEventListener("click", onSummaryChange);
